...
def inception_v3(nlabels, images, pkeep, is_training):
    ...
    with tf.variable_scope("output") as scope:
        ...
        # tf.variables_initializer([weights])
    print("finally")
    return output
...
def levi_hassner_bn(nlabels, images, pkeep, is_training):
    ...
    with tf.variable_scope("LeviHassnerBN", "LeviHassnerBN", [images]) as scope:
        with tf.contrib.slim.arg_scope(
            ...
            with tf.contrib.slim.arg_scope(
                ...
                conv2 = convolution2d(
                    pool1, 256, [5, 5], [1, 1], padding="SAME", scope="conv2"
                )
    ...
    with tf.variable_scope("output") as scope:
        weights = tf.Variable(
            tf.random_normal([512, nlabels], mean=0.0, stddev=0.01), name="weights"
        )
        biases = tf.Variable(
            tf.constant(0.0, shape=[nlabels], dtype=tf.float32), name="biases"
        )
        ...
        with tf.contrib.slim.arg_scope(
            ...
            with tf.contrib.slim.arg_scope(
                ...
                norm1 = tf.nn.local_response_normalization(
                    pool1, 5, alpha=0.0001, beta=0.75, name="norm1"
                )
                conv2 = convolution2d(
                    norm1, 256, [5, 5], [1, 1], padding="SAME", scope="conv2"
                )
                pool2 = max_pool2d(conv2, 3, 2, padding="VALID", scope="pool2")
                norm2 = tf.nn.local_response_normalization(
                    pool2, 5, alpha=0.0001, beta=0.75, name="norm2"
                )
                conv3 = convolution2d(
                    norm2,
                    384,
