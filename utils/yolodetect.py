...
class YOLOBase(ObjectDetector):
    ...
    def _conv_layer(self, idx, inputs, filters, size, stride):
        ...
        return tf.maximum(
            self.alpha * conv_biased, conv_biased, name=str(idx) + "_leaky_relu"
        )
    ...
    def _fc_layer(self, idx, inputs, hiddens, flat=False, linear=False):
        ...
        if linear:
            return tf.add(
                tf.matmul(inputs_processed, weight), biases, name=str(idx) + "_fc"
            )
    ...
    def _iou(self, box1, box2):
        tb = min(box1[0] + 0.5 * box1[2], box2[0] + 0.5 * box2[2]) - max(
            box1[0] - 0.5 * box1[2], box2[0] - 0.5 * box2[2]
        )
        lr = min(box1[1] + 0.5 * box1[3], box2[1] + 0.5 * box2[3]) - max(
    ...
    def draw_rect(self, img, x, y, w, h):
        ...
        cv2.rectangle(
            img,
            (lower_cut[1], lower_cut[0]),
            (upper_cut[1], upper_cut[0]),
            (0, 255, 0),
            2,
        )
...
class PersonDetectorYOLOTiny(YOLOBase):
    ...
    def interpret_output(self, output):
        ...
        offset = np.transpose(
            np.reshape(np.array([np.arange(7)] * 14), (2, 7, 7)), (1, 2, 0)
        )
...
class FaceDetectorYOLO(YOLOBase):
    ...
    def interpret_output(self, output):
        ...
        offset = np.transpose(
            np.reshape(np.array([np.arange(11)] * (2 * 11)), (2, 11, 11)), (1, 2, 0)
        )
